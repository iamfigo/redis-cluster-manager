...
public class RedisClusterManager {
	...
	/*
    private void importMongodb(String KeyPre, String key) {
...
*/
     ...
     * 删除用户信息
     */
    public void userinfoDel(final String filePath) {
        final String[] exportKeyPre = "rpcUserInfo".split(",");
        createExportFile(filePath);
        Iterator<Entry<String, JedisPool>> nodes = cluster.getClusterNodes().entrySet().iterator();
        List<Thread> exportTheadList = new ArrayList<Thread>();
        while (nodes.hasNext()) {
                    ...
                    do {
                        ...
                        for (String key : result) {
                            ...
                            if ("hash".equals(keyType)) {
                                ...
                                sb.append(key).append("->").append(JSONObject.toJSONString(value));
                                writeFile(sb.toString(), "export", filePath);
                                nodeCli.del(key);
                            }
                        }
        ...
        long useTime = System.currentTimeMillis() - writeBeginTime, totalCount = writeCount.get();
        float speed = (float) (totalCount / (useTime / 1000.0));
        System.out.println("scan count:" + scanCount.get() + " del total:" + totalCount + " speed:"
                + speedFormat.format(speed) + " useTime:" + (useTime / 1000.0) + "s");
    ...
    private static AtomicLong live_switchCount = new AtomicLong();
    private static AtomicLong praise_switchCount = new AtomicLong();
    private static AtomicLong follow_switchCount = new AtomicLong();
    private static AtomicLong comment_switchCount = new AtomicLong();

    /**
     * 直播开关count统计
     */
    public void switchCount(final String filePath) {
        createExportFile(filePath);
        Iterator<Entry<String, JedisPool>> nodes = cluster.getClusterNodes().entrySet().iterator();
        List<Thread> exportTheadList = new ArrayList<Thread>();
        while (nodes.hasNext()) {
                    ...
                    do {
                        ...
                        for (String key : result) {
                            ...
                            if (key.startsWith("u_")) {
                                try {
                                    Integer.valueOf(key.substring("u_".length()));
                                    userCount.incrementAndGet();
                                } catch (Exception e) {
                                }
                            } else if (key.startsWith("pushswitch:")) {
                                Map<String, String> switchInfo = nodeCli.hgetAll(key);
                                boolean isWriteLog = false;
                                if ("1".equals(switchInfo.get("live_switch"))) {
                                    live_switchCount.incrementAndGet();
                                    isWriteLog = true;
                                }
                                if ("1".equals(switchInfo.get("praise_switch"))) {
                                    praise_switchCount.incrementAndGet();
                                    isWriteLog = true;
                                }
                                if ("1".equals(switchInfo.get("comment_switch"))) {
                                    comment_switchCount.incrementAndGet();
                                    isWriteLog = true;
                                }
                                if ("1".equals(switchInfo.get("follow_switch"))) {
                                    follow_switchCount.incrementAndGet();
                                    isWriteLog = true;
                                }
                                if (isWriteLog) {
                                    writeFile(key + switchInfo.toString(), "export", filePath);
                                }
                            }
                            if (count % 1000000 == 0) {
                                if (readLastCountTime > 0) {
        ...
        long useTime = System.currentTimeMillis() - writeBeginTime, totalCount = writeCount.get();
        float speed = (float) (totalCount / (useTime / 1000.0));
        String info = "scanCount:" + scanCount.get() + " userCount:" + userCount.get()
                + " live_switchCount:" + live_switchCount.get() + " praise_switchCount:" + praise_switchCount.get()
                + " comment_switchCount:" + comment_switchCount.get() + " follow_switchCount:" + follow_switchCount.get() + " speed:"
                + speedFormat.format(speed) + " useTime:" + (useTime / 1000.0) + "s";
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (args.length > 0) {
            ...
            } else if ("switchCount".equals(cmd)) {
                if (args.length == 2) {
                    rcm.switchCount(args[1]);
                } else {
                    System.out.println("switchCount D:/switchCount.dat");
                }
            ...
            } else if ("userinfoDel".equals(cmd)) {
                if (args.length == 2) {
                    rcm.userinfoDel(args[1]);
                } else {
                    System.out.println("userinfoDel D:/userinfoDel_delete.dat");
                }
