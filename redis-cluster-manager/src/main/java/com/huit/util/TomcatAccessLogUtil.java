...
public class TomcatAccessLogUtil {
    ...
    static long readLine = 0, countLine, readWarnCount;
    ...
    public static void parseData(String data) {
        ...
        try {
            ...
            if (time > warnTime) {
                System.out.println("warnTime->url:" + url + " time:" + time);
    ...
    public static void printWarnStatMap() {
        ...
        while (it.hasNext()) {
            ...
            Long count = entry.getValue().longValue();
            if (count > 0) {
                hitRatio = count / 0.01 / readWarnCount;
            }
            DecimalFormat fmt = new DecimalFormat("#0.00");
            double avgTime = (urlWarnTimeCount.get(cmd).get() / 1.0 / count);
            double avgTotal = (urlTimeCount.get(cmd).get() / 1.0 / urlCount.get(cmd).get());
            double warnRatio = (count / 1.0 / urlCount.get(cmd).get());

            System.out.println(cmd + "->count:" + count + " avgTime:" + fmt.format(avgTime) + " avgTotal:" + fmt.format(avgTotal) + " warnRatio:" + fmt.format(warnRatio) + "% (" + fmt.format(hitRatio) + "%)");
            String csv = cmd + "," + count + "," + fmt.format(avgTime) + "\r\n";
            try {
                fw.write(csv);
            } catch (IOException e) {
